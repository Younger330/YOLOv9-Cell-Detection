...
TOLERANT_LABELS = {0: [9], 1: [7], 2: [8, 30], 4: [15, 29], 5: [30, 11], 7: [1, 8], 8: [2, 7], 9: [0, 19], 11: [5], 13: [21], 14: [19], 15: [29, 4], 18: [20], 19: [9, 14], 20: [18], 21: [13], 25: [28], 28: [25], 29: [15, 4], 30: [2, 5]}
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    # 将相似性矩阵S加入到损失函数
    # compute_loss = ComputeLoss(model, opt.similar_loss, TOLERANT_LABELS, ALPHA, BETA)  # init loss class
...
def parse_opt(known=False):
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'demo/BMblood/BM_IN.yaml', help='dataset.yaml path')
    ...
    parser.add_argument('--name', default='debug/exp', help='save to project/name')
    ...
    parser.add_argument('--augu-cp', action="store_true", help='copy-paste for data augument in training') # 在参数中出现就是true，不出现就是false
